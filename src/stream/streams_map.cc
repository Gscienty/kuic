...
kuic::stream::streams_map::streams_map(
    ...
    , in_uni(
            ...
            [this] (kuic::stream_id_t stream_id) -> kuic::stream::receive_stream * {
                return new kuic::stream::receive_stream(stream_id, this->sender, this->new_flow_controller(stream_id));
            })
    , out_uni(
            is_client ? 2 : 3,
            [this] (kuic::stream_id_t stream_id) -> kuic::stream::send_stream * {
                return new kuic::stream::send_stream(stream_id, this->sender, this->new_flow_controller(stream_id));
            },
