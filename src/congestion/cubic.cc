...
kuic::congestion::cubic(kuic::clock &clock)
    : clock(clock) 
...
kuic::packet_number_t kuic::congestion::congestion_window_after_packet_loss(kuic::packet_number_t current_congestion_window) {
    if (current_congestion_window < this->last_max_congestion_window) {
        this->last_max_congestion_window = kuic::packet_number_t(kuic::congestion::cubif_beta_last_max * float(current_congestion_window));
    }
    else {
...
kuic::packet_number_t kuic::congestion::congestion_window_after_ack(kuic::packet_number_t current_congestion_window, kuic::kuic_time_t delay_min) {
    this->acked_packets_count++;
    kuic::special_clock current_time(this->clock);

    if (this->last_congestion_window == current_congestion_window && current_time - this->last_update_time <= kuic::congestion::cubic_max_time_interval) {
    ...
    kuic::kuic_time_t offset = std::abs<kuic::kuic_time_t>(kuic::kuic_time_t(this->time_to_origin_point) - elapsed_time);

    kuic::packet_number_t delta_congestion_window = kuic::packet_number_t((kuic::congestion::cubic_congestion_window_scale * offset * offset * offset) >> kuic::congestion::cubic_scale);
    ...
    while (true) {
        kuic::packet_number_t required_ack_count = kuic::packet_number_t(float(this->estimated_tcp_congestion_window) / this->alpha());
